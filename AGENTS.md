...
## Requirements on Running Commands

- For any python related commands, such as `python`, `pytest`, `pip`, and `pre-commit`, always run them through `bash .codex/run-in-env.sh ...`
- Forbid running Python commands directly in the repository without the wrapper script, to ensure the correct environment is activated and configured.
